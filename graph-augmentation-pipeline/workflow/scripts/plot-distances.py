...
def plot_histogram(dataset, filename):
	...
	with PdfPages(filename) as pdf:
		...
		plt.yscale('log')
		plt.ylabel('Count')
		plt.xlabel('Distance between augmented graph bubbles and their closest original graph bubble')
		pdf.savefig()
		plt.close()

		# plot the same histogram but only up to 25,000
		plt.figure()
		plt.xlim([0,25000])
		plt.hist(dataset, bins=np.arange(0, 25000 + 100, 100), alpha=0.7)
		plt.yscale('log')
		plt.ylabel('Count')
		plt.xlabel('Distance between augmented graph bubbles and their closest original graph bubble')
		pdf.savefig()
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(prog='bedtools closest -d -a <augmented-graph-bubbles> -b <original-graph-bubbles> | python3 plot-histogram.py', description=__doc__)
